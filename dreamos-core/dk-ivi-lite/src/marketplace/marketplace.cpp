...
void ensureMarketplaceSelectionExists(const QString &marketplaceFilePath) {
    ...
    if (!file.exists()) {
        ...
        if (!parentDir.exists()) {
            qDebug() << "Parent directory doesn't exist, creating" << parentDir.path();
            if (!parentDir.mkpath(".")) {
                qDebug() << "Error: Could not create parent directory" << parentDir.path();
                return;
            }
        }
        ...
        defaultMarketplace["name"] = "Digital Auto Marketplace";
        defaultMarketplace["marketplace_url"] = "https://store-be.digital.auto"; // Fixed: removed %22
        defaultMarketplace["login_url"] = "https://user-mgmt.digital.auto/api/v1/user/login";
...
Q_INVOKABLE void AppAsync::initInstalledAppFromDB()
{
    ...
    // Handle tilde expansion in container root path
    ...
    // Use the new path in ~/.dk/ instead of ./installedapps/
    QString csvPath = expandedContainerRoot + "installedapps.csv";
    ...
    for(int i = 1; i < appList.size(); i++) {
        ...
        // Update the icon path to use the new directory structure
        appInfo.iconPath    = "file:" + expandedContainerRoot + appInfo.foldername + "/" + appList[i][3];
        ...
        QDir appDir(expandedContainerRoot + appInfo.foldername);
        qDebug() << "  App directory exists:" << appDir.exists();
        
        // Check if the icon file exists
        QFileInfo iconInfo(expandedContainerRoot + appInfo.foldername + "/" + appList[i][3]);
...
void AppAsync::appstore_readAppList(const QString searchName, QList<AppListStruct> &AppListInfo) 
{
    ...
    // Create default marketplace file if it doesn't exist
    QFile defaultFile(mpDataPath);
    if (!defaultFile.exists()) {
        qDebug() << "Creating default marketplace file:" << mpDataPath;
        if (defaultFile.open(QIODevice::WriteOnly)) {
            // Create an empty array for marketplace data (not marketplace selection)
            QJsonArray emptyArray;
            QJsonDocument doc(emptyArray);
            defaultFile.write(doc.toJson());
            defaultFile.close();
            qDebug() << "Created empty marketplace_data_installcfg.json file";
        }
    }
    ...
    // Fix URL if it has %22 at the end (encoded double quote)
    if (marketplace_url.endsWith("%22")) {
        marketplace_url = marketplace_url.left(marketplace_url.length() - 3);
        qDebug() << "Fixed marketplace URL, removing %22 (encoded quote). New URL:" << marketplace_url;
    }
    ...
    // Read the JSON file (which now should exist)
    QFile file(mpDataPath);
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qDebug() << "Failed to open file." << mpDataPath;
        return;
    }
...
Q_INVOKABLE void AppAsync::installApp(const int index)
{
    ...
    QString installCfg = "~/.dk/dk_marketplace/" + appId + "_installcfg.json";
    qDebug() << "Install config path:" << installCfg;
    qDebug() << "Install config exists:" << QFileInfo(installCfg).exists();

    QString cmd = "docker kill dk_appinstallservice;docker rm dk_appinstallservice;docker run -d -it --name dk_appinstallservice -v ~/.dk:/app/.dk -v /var/run/docker.sock:/var/run/docker.sock --log-opt max-size=10m --log-opt max-file=3 -v " + installCfg + ":/app/installCfg.json " + "autowrx/dk_appinstallservice:latest";
...
Q_INVOKABLE void AppAsync::removeApp(const int index)
{
    ...
    // Remove entire app folder - update path to use ~/.dk/
    QString cmd;
    cmd.clear();
    cmd = "rm -rf " + expandedContainerRoot + installedAppList[index].foldername;
    qDebug() << cmd;
    system(cmd.toUtf8());
    cmd.clear();
    cmd = "rm -rf " + expandedContainerRoot + installedAppList[index].foldername + ".zip";
    qDebug() << cmd;
    system(cmd.toUtf8());
