...
def main():
    ...
    if result == False:
        ...
        if (DeployTarget == "vip"):
            ...
            cmd = f"sshpass -p '{DK_VIP_PWD}' ssh -o StrictHostKeyChecking=no {DK_VIP_USER}@{DK_VIP_IP} 'docker pull {DK_XIP_IP}:5000/{DockerImageURL} ; mkdir -p /home/.dk/dk_installedservices'"
            result = cmd_execute(cmd)
            if result == False:
                print(f"Error: can't execute {cmd}")
